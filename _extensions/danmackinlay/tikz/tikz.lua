...
local function code_to_figure(conf)
  return function(block)
    ...
    if out_format == 'svg' then
      ...
      -- Inlining bypasses pandoc.Image, which is what would normally carry the
      -- block's `alt` text, so the alt would be dropped from the HTML entirely.
      -- Re-attach it to the <svg> root as an accessible name. (Local patch.)
      local alt_text = stringify(dgr_opt.alt or '')
      if alt_text ~= '' then
        local esc = alt_text
            :gsub('&', '&amp;'):gsub('<', '&lt;'):gsub('>', '&gt;'):gsub('"', '&quot;')
            :gsub('%%', '%%%%') -- protect gsub replacement specials
        svg = svg:gsub('^<svg', '<svg role="img" aria-label="' .. esc .. '"', 1)
      end
